...
def main():
    ...
    if args.last_model is not None and args.last_epoch is not None:
        model.load_model(args.last_model)
        train_from_epoch(model, args.last_epoch + 1)
    else:
        train(model)
...
def train_from_epoch(model, start_epoch):
    model.write_on_log(f"Starting training...")
    ...
    for epoch in range(1, model.get_train_encoder_num_epochs() + 1):
        ...
        if epoch < start_epoch:
            model.write_on_log(f"Skipping epoch {epoch}...")
            model.get_scheduler().step()
            continue
...
def get_args():
    ...
    parser.add_argument("--last_model", type=str, required=False)
    parser.add_argument("--last_epoch", type=int, required=False)
